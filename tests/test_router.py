...
class RouteMessageTests(unittest.TestCase):
    ...
    def test_router_called_on_first_message(self):
        decision = RouteDecision(route="in_scope", standalone_question="Best hikes in Yosemite?", clarifying_question=None, redirect_message=None)
    ...
    def test_only_last_six_history_messages_sent(self):
        self.parse.return_value = parsed(
            RouteDecision(route="in_scope", standalone_question="q", clarifying_question=None, redirect_message=None)
        )
    ...
    def test_ambiguous_without_clarifying_question_raises(self):
        self.parse.return_value = parsed(
            RouteDecision(route="ambiguous", standalone_question="weather this weekend?", clarifying_question=None, redirect_message=None)
        )
    ...
    def test_out_of_scope_without_redirect_message_raises(self):
        self.parse.return_value = parsed(
            RouteDecision(route="out_of_scope", standalone_question="Write a poem", clarifying_question=None, redirect_message=None)
        )
...
class HandleMessageTests(unittest.TestCase):
    def setUp(self):
        ...
        self.generate = self.patch("app.generate_response")
    ...
    def test_in_scope_retrieves_with_standalone_and_generates_with_raw(self):
        ...
        self.route.return_value = RouteDecision(
            route="in_scope", standalone_question="Day 2 of a Yosemite trip?", clarifying_question=None, redirect_message=None
        )
        self.retrieve.return_value = ["chunk"]
        self.generate.return_value = "Day 2 plan"

        self.assertEqual(self.answer("what about day 2?", history), "Day 2 plan")
        self.retrieve.assert_called_once_with("Day 2 of a Yosemite trip?", k=10)
        self.generate.assert_called_once_with("what about day 2?", ["chunk"], recent=history)
